...
def autopad(k, p=None):  # kernel, padding
    ...
    # Pad to 'same'
    if p is None:
        p = k // 2 if isinstance(k, int) else [x // 2 for x in k]  # 차원을 유지하기
    return p
...
class Bottleneck(nn.Module):
    # Standard bottleneck (resnet)
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super(Bottleneck, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c_, c2, 3, 1, g=g)  # padding: 1
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = nn.Conv2d(c1, c_, 1, 1, bias=False)  # 끝에서 concat할 절반 conv (No BN)
        self.cv3 = nn.Conv2d(c_, c_, 1, 1, bias=False)  # No BN
        self.cv4 = Conv(2 * c_, c2, 1, 1)
        self.bn = nn.BatchNorm2d(2 * c_)  # applied to cat(cv2, cv3)
        self.act = nn.LeakyReLU(0.1, inplace=True)
        self.m = nn.Sequential(*[Bottleneck(c_, c_, shortcut, g, e=1.0) for _ in range(n)])  # n: 반복할 bottle neck 횟수

    def forward(self, x):
        y1 = self.cv3(self.m(self.cv1(x)))  # conv(bn) + bottleneck + conv(No bn)
        y2 = self.cv2(x)  # conv(No bn)
        return self.cv4(self.act(self.bn(torch.cat((y1, y2), dim=1))))


class C3(nn.Module):
    # CSP Bottleneck with 3 convolutions
    # 위의 BottleneckCSP보다 약간 간소화 된 CSP (위에서 self.cv3이 없다고 생각)
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
